...
class CompaniesControllerTest < ApplicationSystemTestCase
  ...
  test "City and State check while company Update" do

    # For zip codes present with ZipCodes gem
    visit edit_company_path(@company)
    within("form#edit_company_#{@company.id}") do
      fill_in("company_name", with: "Updated Test Company 1")
    ...
    @company.reload
    assert_equal "Updated Test Company 1", @company.name
    assert_equal "30301", @company.zip_code
    assert_equal "Atlanta", @company.city
    assert_equal "GA", @company.state


    # For zip codes not present with ZipCodes gem
    visit edit_company_path(@company)
    within("form#edit_company_#{@company.id}") do
      fill_in("company_name", with: "Updated Test Company 2")
      fill_in("company_zip_code", with: "123")
      click_button "Update Company"
    end
    ...
    @company.reload
    assert_equal "Updated Test Company 2", @company.name
    assert_equal "123", @company.zip_code
  ...
  test "City and State check while company creation" do
    visit new_company_path
